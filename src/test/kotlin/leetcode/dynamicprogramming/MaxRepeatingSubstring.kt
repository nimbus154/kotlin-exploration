...
value k where word is k-repeating in sequence. If word is not a substring of sequence,
...
class MaxRepeatingSubstringSolution {
    fun maxRepeating(sequence: String, word: String): Int {
        var k = 0
        val kWord = StringBuilder(word)
...
class MaxRepeatingSubstringTest {
    ...
    fun test(sequence: String, word: String, expected: Int) {
        val actual = MaxRepeatingSubstringSolution().maxRepeating(sequence, word)
        assertEquals(expected, actual, "seq=$sequence, word=$word")
    }
}
